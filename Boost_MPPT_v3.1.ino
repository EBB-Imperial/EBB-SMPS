...
//          MPPT
          ...
          {   // deltaP > 0
            if(vb > Voltage_prev) 
              closed_loop = closed_loop - 0.001;  //deltaV > 0 & deltaP > 0 => positive slope => V+
            else
              closed_loop = closed_loop + 0.001;  //deltaV < 0 & deltaP > 0 => negative slope => V-
          }
          else  
          {   //deltaP < 0
            if(vb > Voltage_prev) 
...
//          //Memory 
          ...
          //PWM
          closed_loop=saturation(closed_loop,0.99,0.2);
...
//  vref = sensorValue2 * (4.096 / 1023.0); // Convert the Vref sensor reading to volts
  ...
  if (Boost_mode == 1){
    ...
    dutyref = saturation(sensorValue2 * (1.0 / 1023.0),0.99,0.20);
  }else{
    iL = current_mA/1000.0;
