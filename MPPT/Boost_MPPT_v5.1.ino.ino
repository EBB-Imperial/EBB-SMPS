...
//          if(Power_now - Power_prev <)
          ...
          if(rate_of_change_of_power<threshold_low && Power_now!=0 && Power_prev != 0){
            delta = delta_min;
            }
           else if(rate_of_change_of_power > threshold_low){
            delta = delta; 
            } 
           else if (Power_prev == 0 && Power_now == 0) {
            delta = delta;
            }
...
//  vref = sensorValue2 * (4.096 / 1023.0); // Convert the Vref sensor reading to volts
  ...
  if (Boost_mode == 1){
    ...
    dutyref = saturation(sensorValue2 * (1.0 / 1023.0),0.99,0.05);
  }else{
    iL = current_mA/1000.0;
